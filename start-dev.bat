...
powershell -NoProfile -Command ^
  "$ok = $false; for ($i = 0; $i -lt 60; $i++) { try { Invoke-WebRequest -UseBasicParsing http://127.0.0.1:8001/api/config -TimeoutSec 1 | Out-Null; $ok = $true; break } catch { Start-Sleep -Milliseconds 500 } }; if (-not $ok) { Write-Host 'Backend non disponible sur 8001 apres attente.' }"
